...
import me.ccbluex.liquidbounce.features.module.modules.player.Blink
import me.ccbluex.liquidbounce.features.module.modules.render.FreeCam
...
@ModuleInfo(name = "KillAura", description = "Test", category = ModuleCategory.COMBAT)
class LegitAura : Module() {
    ...
    fun onMotion(event: MotionEvent) {
        ...
        updateHitable()
        val target = currentTarget ?: discoveredTargets.getOrNull(0) ?: return
        ...
        return
        // AutoBlock
        if (autoBlockValue.get() == "Range" && discoveredTargets.isNotEmpty() && (!autoBlockPacketValue.equals("AfterAttack") || discoveredTargets.any { it.getDistanceToEntityBox(it) > maxRange }) && canBlock)
            {
            ...
            if (mc.thePlayer!!.hurtTime > 8) {
                ...
                if (mc.thePlayer!!.hurtTime == 2) {
                    ...
                    if (discoveredTargets.isNotEmpty() && !blockingStatus) {
                        val target = currentTarget ?: discoveredTargets.first()
                        startBlocking(target, interactAutoBlockValue.get() && (mc.thePlayer!!.getDistanceToEntityBox(target) < maxRange))
                        blockingStatus = true
                    }
                    if (clicks > 0)
    ...
    private fun runAttack() {
        ...
        if (!failHit) {
            ...
            if (openInventory) {
                mc2.connection!!.networkManager.sendPacket(CPacketCloseWindow())
            }
    ...
    private fun updateTarget() {
        ...
        for (entity in mc.theWorld!!.loadedEntityList) {
            if (entity !is IEntityLivingBase || !EntityUtils.isSelected(
                    entity,
                    true
        ...
        when (priorityValue.get().toLowerCase()) {
            ...
            "health" -> discoveredTargets.sortBy { it.health + it.activePotionEffects.size } // Sort by health
        ...
        inRangeDiscoveredTargets.clear()
        inRangeDiscoveredTargets.addAll(0, discoveredTargets.filter { mc.thePlayer!!.getDistanceToEntityBox(it) <= rangeValue.get() })
    ...
    private fun runSwing() {
        ...
        if (swing.equals("packet", true)) {
            mc2.connection!!.networkManager.sendPacket(CPacketAnimation())
    ...
    private fun attackEntity(entity: IEntityLivingBase) {
        ...
        runSwing()
        packetSent = true
        classProvider.createCPacketUseEntity(entity, ICPacketUseEntity.WAction.ATTACK)
    ...
    private fun preAttack() {
        if (mc.thePlayer!!.isBlocking || blockingStatus) {
            when (autoBlockPacketValue.get().toLowerCase()) {
                ...
                "oldintave" -> {
                    mc2.connection!!.networkManager.sendPacket(CPacketHeldItemChange(mc.thePlayer!!.inventory.currentItem % 8 + 1))
                    mc2.connection!!.networkManager.sendPacket(CPacketHeldItemChange(mc.thePlayer!!.inventory.currentItem))
                    blockingStatus = false
                }
    ...
    private fun postAttack(entity: IEntityLivingBase) {
    ...
    private fun swingKeepSprint(entity: IEntityLivingBase) {
        if (keepSprintValue.get()) {
            // Enchant Effect
            if (EnchantmentHelper.getModifierForCreature(mc2.player.getHeldItem(EnumHand.MAIN_HAND), mc2.player.creatureAttribute) > 0F) {
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        // 可以被看见
        if (entityFov <= mc2.gameSettings.fovSetting) lastCanBeSeen = true
        ...
        val boundingBox = if (rotationModeValue.get() == "Test") entity.entityBoundingBox
        else RotationUtils.getCenter(entity.entityBoundingBox)
    ...
    private fun updateHitable() {
        ...
        val entityDist = mc.thePlayer!!.getDistanceToEntityBox(currentTarget as IEntity)
        canSwing = entityDist < swingRangeValue.get() && (currentTarget as IEntityLivingBase).hurtTime <= hurtTimeValue.get()
     ...
     * Start blocking
     */
    private fun startBlocking(interactEntity: IEntityLivingBase, interact: Boolean) {
    ...
    //    mc2.connection!!.networkManager.sendPacket(CPacketPlayerDigging(mc.thePlayer!!.inventory.currentItem))
        blockingStatus = true
        packetSent = true
    }
            ...
            when (markValue.get().toLowerCase()) {
                ...
                "jello" -> {
                    ...
                    for(i in 0..360 step 7){
                        points.add(Vec3d(posX - sin(i * Math.PI / 180F) * radius,posY+height*drawPercent,posZ + cos(i * Math.PI / 180F) * radius))
                    }
                    points.add(points[0])
                    //draw
                    mc.entityRenderer.disableLightmap()
                    ...
                    for(i in 0..20) {
                        ...
                        GL11.glVertex3d(
                            firstPoint.x - mc.renderManager.viewerPosX, firstPoint.y - moveFace - min - mc.renderManager.viewerPosY,
                            firstPoint.z - mc.renderManager.viewerPosZ
                        )
                        GL11.glColor4f(1F, 1F, 1F, 0.7F*(i/20F))
                        for (vec3 in points) {
                            GL11.glVertex3d(
                                vec3.x - mc.renderManager.viewerPosX, vec3.y - moveFace - min - mc.renderManager.viewerPosY,
                                vec3.z - mc.renderManager.viewerPosZ
                            )
                        }
    ...
    private val cancelRun: Boolean
        get() = mc2.player.isSpectator || !isAlive(mc.thePlayer!!)
