...
import me.ccbluex.liquidbounce.utils.RotationUtils.getVectorForRotation
import me.ccbluex.liquidbounce.utils.RotationUtils.serverRotation
...
object RaycastUtils : MinecraftInstance() {
    ...
    fun raycastEntity(
        ...
        entityFilter: (IEntity?) -> Boolean
    ): IEntity? {
        val renderViewEntity = mc.renderViewEntity

        if (renderViewEntity != null && mc.theWorld != null) {
            var blockReachDistance = range
            val eyePosition = renderViewEntity.getPositionEyes(mc.timer.renderPartialTicks)
