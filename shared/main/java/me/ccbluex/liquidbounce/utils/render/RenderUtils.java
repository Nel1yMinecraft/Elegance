...
;
import me.ccbluex.liquidbounce.api.enums.WDefaultVertexFormats;
...
import me.ccbluex.liquidbounce.api.minecraft.client.render.IWorldRenderer;
...
import me.ccbluex.liquidbounce.utils.blur.ShaderUtil;
...
public final class RenderUtils extends MinecraftInstance {
    ...
    public static void drawBlockBox(final WBlockPos blockPos, final Color startColor, final Color endColor, final boolean outline) {
        final IRenderManager renderManager = mc.getRenderManager();
        final ITimer timer = mc.getTimer();
        ...
        for (int i = 0; i < steps; i++) {
            ...
            int alpha = (int) (startColor.getAlpha() + (endColor.getAlpha() - startColor.getAlpha()) * percent);
        ...
        if (outline) {
            glLineWidth(1F);
            enableGlCap(GL_LINE_SMOOTH);
            glColor(endColor); // 使用结束颜色作为线框的颜色
    ...
    public static void originalRoundedRect(float paramXStart, float paramYStart, float paramXEnd, float paramYEnd, float radius, int color) {
        ...
        Tessellator tessellator = Tessellator.getInstance();
        IWorldRenderer worldrenderer = (IWorldRenderer) tessellator.getBuffer();
