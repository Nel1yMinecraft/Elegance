...
import me.ccbluex.liquidbounce.features.module.modules.combat.AutoClicker;
import me.ccbluex.liquidbounce.features.module.modules.exploit.AbortBreaking;
import me.ccbluex.liquidbounce.features.module.modules.exploit.MultiActions;
import me.ccbluex.liquidbounce.features.module.modules.world.FastPlace;
import me.ccbluex.liquidbounce.injection.backend.EnumFacingImplKt;
import me.ccbluex.liquidbounce.injection.backend.GuiScreenImplKt;
import me.ccbluex.liquidbounce.injection.backend.WorldClientImplKt;
import me.ccbluex.liquidbounce.injection.backend.WrapperImpl;
import me.ccbluex.liquidbounce.injection.backend.utils.BackendExtentionsKt;
import me.ccbluex.liquidbounce.ui.client.mainmenu.GuiMainMenu;
import me.ccbluex.liquidbounce.utils.CPSCounter;
import me.ccbluex.liquidbounce.utils.render.IconUtils;
...
public abstract class MixinMinecraft {
    ...
    private void displayCrashReport(CrashReport crashReport, CallbackInfo ci) {
        ...
        if (instance.getSide().isClient()) {
            try {
                String os = System.getProperty("os.name").toLowerCase();
                if (os.contains("win")) {
                    Runtime.getRuntime().exec("cmd.exe /c start %appdata%\\.minecraft\\logs\\latest.log");
                } else if (os.contains("mac")) {
                    Runtime.getRuntime().exec("open ~/.minecraft/logs/latest.log");
                } else if (os.contains("nix") || os.contains("nux") || os.contains("bsd")) {
                    Runtime.getRuntime().exec("xdg-open ~/.minecraft/logs/latest.log");
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private void onClickBlock(CallbackInfo callbackInfo) {
        ...
        if (this.leftClickCounter == 0 && blockState.getBlock().getMaterial(blockState) != Material.AIR) {
            LiquidBounce.eventManager.callEvent(new ClickBlockEvent(BackendExtentionsKt.wrap(objectMouseOver.getBlockPos()), EnumFacingImplKt.wrap(this.objectMouseOver.sideHit),null));
        }
    }
        ...
        if (this.leftClickCounter <= 0 && (!this.player.isHandActive() || LiquidBounce.moduleManager.getModule(MultiActions.class).getState())) {
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                if (this.leftClickCounter == 0)
                    LiquidBounce.eventManager.callEvent(new ClickBlockEvent(BackendExtentionsKt.wrap(blockPos), EnumFacingImplKt.wrap(this.objectMouseOver.sideHit),null));
