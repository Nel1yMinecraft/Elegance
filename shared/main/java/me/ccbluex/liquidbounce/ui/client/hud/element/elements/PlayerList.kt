...
class PlayerList : Element() {
    ...
    private val rainbowList =
        ListValue("Rainbow", arrayOf("Off", "CRainbow", "Sky", "LiquidSlowly", "Fade", "Gident"), "Off")
    ...
    override fun drawElement(): Border {
        ...
        var playerList: MutableList<IEntityPlayer> =
            mc.theWorld!!.playerEntities.filter { !AntiBot.isBot(it) && it != mc.thePlayer }.toMutableList()
        nameLength = font.getStringWidth("Name (${playerList.size})").toFloat()

        when (sortValue.get()) {
            "Alphabet" -> playerList.sortWith(compareBy { it.name!!.toLowerCase() })
            "Distance" -> playerList.sortWith(Comparator { a, b ->
        ...
        playerList.filter { !teams.isInYourTeam(it) }.forEach {
            ...
            if (font.getStringWidth("${decimalFormat3.format(mc.thePlayer!!.getDistanceToEntityBox(it))}m") > distLength)
                distLength = font.getStringWidth("${decimalFormat3.format(mc.thePlayer!!.getDistanceToEntityBox(it))}m")
                    .toFloat()
        }
                ...
                RenderUtils.drawGradientSideways(
                    ...
                    when (rainbowType) {
                        ...
                        "Gident" -> RenderUtils.getGradientOffset(
                            ...
                            Color(HUD.redValue2.get(), HUD.greenValue2.get(), HUD.blueValue2.get(), 1),
                            (Math.abs(
                                System.currentTimeMillis() / HUD.gidentspeed.get()
        ...
        playerList.forEachIndexed { index, player ->
            ...
            font.drawString(
                "${mc.thePlayer?.let { decimalFormat3.format(it.getDistanceToEntityBox(player)) }}m",
                5F + nameLength + 10F,
